...
class Shapes:
    ...
    def add_cylinder(self, init_pos, radius, height, n_particles=10000, color=None, init_rot=None):
        p = np.random.normal(size=(n_particles, 3))
        p[:, 1] = 0.
        p /= np.linalg.norm(p, axis=-1, keepdims=True)
        u = np.random.random(size=(n_particles, 1)) ** (1. / 2)
        p = p * u * radius
        p[:, 1] = np.random.random(size=(n_particles, )) * height - height / 2
