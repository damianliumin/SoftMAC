...
class TransportLoss:
    def __init__(self, cfg, mpm_sim):
        ...
        self.n_particles_per_controller = self.n_particles // 2
        ...
        self.min_dist_2 = ti.field(dtype=dtype, shape=(), needs_grad=True)
    ...
    def compute_pose_loss_kernel(self, f: ti.i32):
        self.pose_loss[None] += 1.0 * (self.rigid.position[f][0] - ti.static(self.target[0])) ** 2
        self.pose_loss[None] += 1.0 * (self.rigid.position[f][1] - ti.static(self.target[1])) ** 2
        self.pose_loss[None] += 1.0 * (self.rigid.position[f][2] - ti.static(self.target[2])) ** 2
        # self.pose_loss[None] += 1.0 * (self.rigid.rotation[f][0] - ti.cos(np.pi / 8)) ** 2
    ...
    def compute_contact_distance_kernel(self, f: ti.i32):
        for i in range(self.n_particles_per_controller):
            ...
            dist_2 = ti.max(self.l2_dist(f, i + self.n_particles_per_controller) - 0.01, 0.) # controller 2
            ti.atomic_min(self.min_dist_2[None], dist_2)

            # dist_1 = ti.max(self.rigid.sdf(f, self.particle_x[f, i]), 0.) # controller 1
            # ti.atomic_min(self.min_dist_1[None], dist_1)
            # dist_2 = ti.max(self.rigid.sdf(f, self.particle_x[f, i + self.n_particles_per_controller]), 0.) # controller 2
            # ti.atomic_min(self.min_dist_2[None], dist_2)

    @ti.kernel
    def compute_contact_loss_kernel(self, f: ti.i32):
        self.contact_loss[None] += self.min_dist_1[None] ** 2 + self.min_dist_2[None] ** 2
    ...
    def clear_losses(self):
        ...
        self.min_dist_2[None] = 1e6
        self.min_dist_2.grad[None] = 0
